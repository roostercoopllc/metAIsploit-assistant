...
def splice_out_file(llm_resp: str) -> List[LlmFileOutput]:
    ...
    for parse in parsed:
        if SupportedScripts.BASH.value in parse[:10]:
            ...
            scripts.append(
                LlmFileOutput(file_type=SupportedScripts.RUBY.value, content=parse[5:])
            )
    return scripts
...
def save_response_output_to_file(
    ...
    if msf_root:
        ...
        Path(write_loc).parent.mkdir(parents=True, exist_ok=True)
    else:
        print("No METASPLOIT_ROOT env var found")
